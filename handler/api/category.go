...
func (c *categoryAPI) CreateNewCategory(w http.ResponseWriter, r *http.Request) {
	...
	request.UserID = idLogin

	result, err := c.categoryService.StoreCategory(r.Context(), &request)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(entity.NewErrorResponse("error internal server"))
		return
	...
	json.NewEncoder(w).Encode(entity.NewCategoryResponse(idLogin, result.ID, "success create new category"))
}
...
func (c *categoryAPI) DeleteCategory(w http.ResponseWriter, r *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(entity.NewCategoryResponse(userId, categoryId, "success delete category"))
}
